...
class BaseT2VDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print('prompts:', self.prompts, 'videos:', self.videos)
        self.device = device
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        # print(f"prompt: {prompt}, video: {video}")
        ...
        encoded_video_path = video_latent_dir / (video.name.replace('.', '_') + ".safetensors")
        ...
        return {
            ...
            "datatype": "image" if video.suffix.lower() in [".jpg", ".jpeg", ".png", ".bmp", ".gif", ".webp"] else "video",
            "video_metadata": {
...
class T2VDatasetWithResize(BaseT2VDataset):
    ...
    @override
    def preprocess(self, video_path: Path) -> torch.Tensor:
        if video_path.suffix.lower() in [".jpg", ".jpeg", ".png", ".bmp", ".gif", ".webp"]:
            frames = preprocess_image_with_resize(
                video_path,
                self.height,
                self.width,
            )
            frames = frames.unsqueeze(0)  # Add frame dimension
            return frames
        else:
