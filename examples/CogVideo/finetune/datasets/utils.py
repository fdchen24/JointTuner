...
def load_videos(video_path: Path) -> List[Path]:
    with open(video_path, "r", encoding="utf-8") as file:
        return [
            Path(line.strip()) for line in file.readlines() if len(line.strip()) > 0
        ]
...
def preprocess_video_with_resize(
    ...
    """
    Loads and resizes a single video or image.

    The function processes the input through these steps:
      ...
      3. If input is image, repeat it to create max_num_frames
    ...
    # Ensure resizing even if Decord already does some resize
    frames = torch.stack([resize(f, size=(height, width)) for f in frames], dim=0)
    return frames
