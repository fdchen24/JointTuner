...
from utils.lora import extract_lora_ups_down
...
def inference(
    ...
    with torch.autocast(device, dtype=torch.half):
        # prepare models
        pipe, unet = initialize_pipeline(model, device, xformers, sdp, lora_path,  lora_rank, lora_scale)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--negative-prompt", type=str, default="bad anatomy, extra limbs, ugly, deformed, noisy, blurry, distorted, out of focus,  poorly drawn face, poorly drawn hands, missing fingers", help="Text prompt to condition against")
    ...
    parser.add_argument("-s", "--num-steps", type=int, default=30, help="Number of diffusion steps to run per frame.")  # default 30
    ...
    parser.add_argument("-lp", "--lora_path", type=str, default=None, help="Path to Low Rank Adaptation checkpoint file (defaults to empty string, which uses no LoRA).")
